...
from utils.image_gen import genera_prompt_visuale, genera_seed_volto, genera_didascalia, query_image_model, rileva_sesso
...
def render_card(label: str, r: dict, key_suffix: str = "", ambiente: str = "", secolo: str = ""):
    if "errore" in r:
    ...
    with col_info:
        ctx_info = ""
        if ambiente: ctx_info += f" &nbsp;·&nbsp; {ambiente}"
        if secolo:   ctx_info += f" &nbsp;·&nbsp; {secolo}"
        st.markdown(
            f'<div class="card-elemento">{r["elemento"]}</div>'
            f'<div style="font-size:.95rem;color:rgba(44,24,16,.5)">'
            f'{label} &nbsp;·&nbsp; {r["eta"]} anni{ctx_info}</div>',
            unsafe_allow_html=True,
        )
...
# ── HELPER: sezione immagini (fuori da render_card per compatibilità mobile) ──

def render_immagini(label: str, r: dict, key_suffix: str,
                    ambiente: str = "", secolo: str = ""):
    """
    Genera e mostra le 3 immagini FLUX.
    Tenuta separata da render_card perché st.toggle e st.button
    dentro expander + HTML custom non vengono renderizzati su Android.
    """
    import base64

    st.markdown("---")
    # Titolo sezione — puro st.markdown senza HTML annidato
    st.markdown("#### 🖼️ Generazione immagini")

    if not st.secrets.get("HF_TOKEN"):
        st.info("Aggiungi HF_TOKEN nei Secrets per abilitare la generazione immagini.")
        return
    ...
    if genera:
        ...
        imgs_generati = {}
        p_frontale = genera_prompt_visuale(
            r, tipo="frontale", sesso=sesso,
            ambiente=ambiente, secolo=secolo,
        )
        with st.spinner("FLUX: Ritratto frontale…"):
            imgs_generati["frontale"] = ("Ritratto frontale", p_frontale,
                                          query_image_model(p_frontale))

        seed_volto = genera_seed_volto(r, sesso=sesso, barba=barba)
        for tipo, titolo in [("laterale", "Profilo laterale"),
                              ("corpo",   "Figura intera")]:
            prompt = genera_prompt_visuale(
                r, tipo=tipo, sesso=sesso,
                seed_volto=seed_volto,
                ambiente=ambiente, secolo=secolo,
            )
            with st.spinner(f"FLUX: {titolo}…"):
                imgs_generati[tipo] = (titolo, prompt, query_image_model(prompt))
    ...
    if ss_key in st.session_state:
        ...
        # Su mobile: una colonna per volta (st.columns non funziona bene su schermi stretti)
        for tipo in ["frontale", "laterale", "corpo"]:
            ...
            if img_bytes:
                ...
                st.markdown(
                    f'<a href="data:image/png;base64,{b64}"'                    f' download="{fname}"'                    f' style="display:inline-block;margin-bottom:1rem;'                    f'font-size:.9rem;color:#8b1a1a;'                    f'border:1px solid rgba(139,26,26,.35);'                    f'border-radius:3px;padding:.3rem 1rem;'                    f'text-decoration:none;">'                    f'⬇ Scarica {titolo.lower()}</a>',
                    unsafe_allow_html=True,
                )
            else:
        ...
        with zipfile.ZipFile(zip_buf, "w", zipfile.ZIP_DEFLATED) as zf:
            for tipo in ["frontale", "laterale", "corpo"]:
                _, _, img_bytes = imgs_salvati[tipo]
                if img_bytes:
                    zf.writestr(f"{nome_base}-{tipo}.png", img_bytes)
        zip_buf.seek(0)
        st.download_button(
            label="⬇ Scarica tutte e 3 (.zip)",
            data=zip_buf,
...
with tab_singolo:
    ...
    c3, c4 = st.columns(2)
    with c3:
        ambiente_input = st.text_input("Ambiente geografico", placeholder="es. Europa, Asia orientale, Nord Africa")
    with c4:
        secolo_input = st.text_input("Secolo", placeholder="es. XXI, XIX, Rinascimento")
...
""", unsafe_allow_html=True)
    ...
    if uploaded is not None:
        ...
        if not schede:
            ...
            zip_buf = io.BytesIO()
            with zipfile.ZipFile(zip_buf, "w", zipfile.ZIP_DEFLATED) as zf:
                for label, r, _a, _s in risultati:
                    zf.writestr(
            ...
            with col2:
                st.download_button(
                    ...
                    data="\n\n".join(profilo_to_txt(l, r) for l, r, _a, _s in risultati).encode("utf-8"),
