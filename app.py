...
def render_card(label: str, r: dict, key_suffix: str = "", ambiente: str = "", secolo: str = ""):
    ...
    # ── Generazione immagini FLUX ──────────────────────────────────────────
    st.markdown("---")
    st.markdown(
        '<div style="display:flex;align-items:center;gap:.6rem;margin-bottom:.6rem">'
        '<span style="font-size:1.5rem">🖼️</span>'
        '<span style="font-family:Noto Serif SC,serif;font-size:1.1rem;'
        'color:#8b1a1a;letter-spacing:.05em;font-weight:600">'
        'Generazione immagini</span></div>',
        unsafe_allow_html=True,
    )
        ...
        if not st.secrets.get("HF_TOKEN"):
            ...
            if st.button("🖼️ Genera le 3 immagini", key=f"img_{key_suffix}"):
                ...
                with st.spinner("FLUX: Ritratto frontale…"):
                    img_frontale = query_image_model(p_frontale)
                imgs_generati["frontale"] = ("Ritratto frontale", p_frontale, img_frontale)

                # ── 2. Rileva barba dal frontale (solo uomini) ────────────
                # Non possiamo analizzare l'immagine generata, quindi
                # usiamo l'elemento come proxy: Metallo/Legno/Terra → barba possibile
                elemento = r.get("elemento", "")
                barba = sesso == "M" and elemento in ("Metallo", "Legno", "Terra", "Capricorno")

                # ── 3. Laterale e corpo con seed + ambiente/secolo ────────
