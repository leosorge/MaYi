    ...
    if genera_immagini:
        if not st.secrets.get("HF_TOKEN"):
            ...
            if st.button("🖼️ Genera le 3 immagini", key=f"img_{key_suffix}"):
                ...
                # ── 1. Frontale: genera per primo ────────────────────────
                p_frontale = genera_prompt_visuale(r, tipo="frontale", sesso=sesso)
                ...
                # ── 2. Laterale e corpo: aggiunge seed_volto per coerenza
                # Il seed_volto è la descrizione anatomica del volto usata
                # nel frontale, reinserita nei prompt successivi come ancoraggio.
                seed_volto = genera_seed_volto(r, sesso=sesso)

                for tipo, titolo in [("laterale", "Profilo laterale"),
                                     ("corpo",    "Figura intera")]:
                    prompt = genera_prompt_visuale(
                        r, tipo=tipo, sesso=sesso, seed_volto=seed_volto
                    )
