...
_CORPO_EN = {
    "Metallo": "slender upright figure, medium height, large-boned with lean limbs, athletic build",
    "Legno":   "very slim slender figure, tall and thin, long limbs, lean build, NOT overweight, NOT heavyset",
    "Terra":   "stocky heavyset compact figure, short thick neck, wide waist",
    "Fuoco":   "petite narrow-shouldered figure, slightly wider hips, small hands and feet",
    "Acqua":   "round heavyset full figure, prominent rounded belly, narrow sloping shoulders",
}
_VOLTO_EN = {
...
def genera_seed_volto(risultato: dict, sesso: str = "M", barba: bool = False) -> str:
    """
    ...
    barba_str = ", beard" if (sesso == "M" and barba) else ""
    return (
        f"same {genere} as in the frontal portrait: "
        f"{eta_en}, {volto}, {carnagione}{barba_str}"
    )
...
def genera_prompt_visuale(risultato: dict, tipo: str = "frontale",
    ...
    ctx_parts = []
    if ambiente: ctx_parts.append(f"from {ambiente}")
    if secolo:   ctx_parts.append(f"{secolo} century setting, period-appropriate clothing and styling")
    ctx = ", ".join(ctx_parts)
    ...
    if tipo == "frontale":
        ...
        return (
            f"{_STILE}. "
            f"Frontal face portrait of a {genere}, {eta_en}.{ctx_str} "
        ...
        return (
            f"{_STILE}. "
            f"Side profile portrait, head and shoulders, {eta_en}.{seed}{ctx_str} "
        ...
        return (
            f"{_STILE}. "
            f"Full body shot, entire body visible from head to feet, "
            f"feet and shoes visible at bottom of frame, {eta_en}.{seed}{ctx_str} "
            f"Body build: {corpo}. "
            f"Posture: {movimenti}. "
