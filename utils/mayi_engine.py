...
def parse_file_multi(
    ...
    Campi riconosciuti per scheda (case-insensitive):
      ETA: 38, ETÀ: 38
      AMBIENTE: Europa
      SECOLO: XXI
    Se assenti: eta_default, ambiente="", secolo=""

    Returns
    -------
    schede : list of (label, testo_pulito, eta, ambiente, secolo)
    errori : list of str
    """
    ...
    for label, corpo in blocchi:
        ...
        # AMBIENTE
        m_amb = re.search(r"ambiente\s*[:=]\s*(.+)", corpo, re.IGNORECASE)
        if m_amb:
            ambiente = m_amb.group(1).strip()
            corpo = re.sub(r"ambiente\s*[:=]\s*.+", "", corpo, flags=re.IGNORECASE).strip()
        else:
            ambiente = ""

        # SECOLO
        m_sec = re.search(r"secolo\s*[:=]\s*(.+)", corpo, re.IGNORECASE)
        if m_sec:
            secolo = m_sec.group(1).strip()
            corpo = re.sub(r"secolo\s*[:=]\s*.+", "", corpo, flags=re.IGNORECASE).strip()
        else:
            secolo = ""
        ...
        schede.append((label, corpo, eta, ambiente, secolo))
